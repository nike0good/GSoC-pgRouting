...
#include "drivers/bar/bar_driver.h"

PG_MODULE_MAGIC;

PGDLLEXPORT Datum many_to_many_bar(PG_FUNCTION_ARGS);
PG_FUNCTION_INFO_V1(many_to_many_bar);
...
process(
    ...
    if (only_cost) {
        time_msg("processing pgr_barCost", start_t, clock());
    } else {
        time_msg("processing pgr_bar", start_t, clock());
    }
...
PGDLLEXPORT Datum
many_to_many_bar(PG_FUNCTION_ARGS) {
